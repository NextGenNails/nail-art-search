...
// Safe portfolio system - works without database dependencies
// Perfect for testing when database issues occur
...
export default async function handler(
  ...
  try {
    ...
    console.log(`⚡ Safe portfolio loading for ${artistId} (no database dependencies)`)
    ...
    console.log(`✅ Safe portfolio: ${images.length} unique images for ${artistId} (database-independent)`)

    res.status(200).json({
      artist_id: artistId,
      total_images: images.length,
      images: images,
      source: "safe_static_portfolio",
      loading_time: "fast",
      unique_images: true,
      database_independent: true,
      note: "Using safe fallback portfolio - works without database dependencies"
    })

  } catch (error) {
    console.error('Safe images API error:', error)
    res.status(500).json({ 
