...
export default function ArtistProfile() {
  ...
  useEffect(() => {
    if (artistId) {
      // Mock vendor data based on artistId
      const mockVendors = {
        'ariadna': {
      ...
      // Mock images for each vendor (20 images each)
      const mockImages = {
        'ariadna': Array.from({ length: 20 }, (_, i) => ({
          id: `batch_${Math.floor(i/2) + 1}_${i % 2 === 0 ? 0 : 3}`,
          image_url: `https://images.unsplash.com/photo-${1632345031435 + i}?w=400&h=400&fit=crop&crop=center`,
          style: i % 4 === 0 ? "3D Sculptural" : i % 4 === 1 ? "Floral 3D" : i % 4 === 2 ? "Character Art" : "Abstract Mixed Media",
          colors: i % 3 === 0 ? "Multi-color" : i % 3 === 1 ? "Pink, Gold" : "Blue, White",
        ...
        'mia': Array.from({ length: 20 }, (_, i) => ({
          ...
          image_url: `https://images.unsplash.com/photo-${1604654894610 + i}?w=400&h=400&fit=crop&crop=center`,
          style: i % 4 === 0 ? "Classic French" : i % 4 === 1 ? "Acrylic Extensions" : i % 4 === 2 ? "Dip Powder" : "Gel Manicure",
          colors: i % 3 === 0 ? "Natural, White" : i % 3 === 1 ? "Pink, Nude" : "Clear, Natural",
          filename: `mia_design_${i + 1}.jpg`,
          similarity_score: 0.80 + (Math.random() * 0.15)
        }))
      }

      const vendorKey = artistId as string
      setVendor(mockVendors[vendorKey as keyof typeof mockVendors] || null)
      setImages(mockImages[vendorKey as keyof typeof mockImages] || [])
      setIsLoading(false)
    }
  }, [artistId])
